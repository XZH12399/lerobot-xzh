...
def main():
    ...
    for tag, gates in EXPERIMENTS:
        ...
        policy.config.use_stepwise_base_gate = True
        policy.config.base_gate_values = tuple(gates)
        if hasattr(policy, 'model') and hasattr(policy.model, 'config'):
            policy.model.config.inference_base_alpha_override = 1.5
            policy.model.config.use_stepwise_base_gate = True
            policy.model.config.base_gate_values = tuple(gates)

        logging.info('start %s gates=%s', tag, gates)
        ...
        info['base_gate_values'] = list(gates)
        ...
        with open(output_root / 'pi0_residual_everystep_gateAB_ckpt050000_summary.json', 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2)
        logging.info('done %s overall=%s', tag, overall)

    from lerobot.envs.utils import close_envs
    close_envs(envs)
    logging.info('gate AB eval finished')
