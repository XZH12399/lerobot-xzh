...
# Repro script for LeRobot pi0 + PushT sanity training.
...
JOB_NAME="${JOB_NAME:-pi0_pusht_sanity}"
RESUME="${RESUME:-false}"
...
if [[ "$RESUME" == "true" ]]; then
  if [[ -z "$TRAIN_RUN_DIR" ]]; then
    TRAIN_RUN_DIR="$(ls -1dt "$OUTPUT_ROOT"/pi0_pusht_* "$PROJECT_DIR"/outputs/pi0_pusht_* 2>/dev/null | head -n 1 || true)"
  fi
  OUT_DIR="${OUT_DIR:-$TRAIN_RUN_DIR}"
  LOG_FILE="${LOG_FILE:-$PROJECT_DIR/logs/pi0_pusht_resume_${TS}.log}"
else
  OUT_DIR="${OUT_DIR:-$OUTPUT_ROOT/pi0_pusht_${TS}}"
  LOG_FILE="${LOG_FILE:-$PROJECT_DIR/logs/pi0_pusht_${TS}.log}"
fi
