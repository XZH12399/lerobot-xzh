...
class PI05Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI05MemoryConfig, rtc_processor: RTCProcessor | None = None):
        ...
        # Minimal explicit history memory baseline modules.
        ...
        self.memory_time_embedding = None
        if config.use_time_embedding_in_memory:
            self.memory_time_embedding = nn.Embedding(config.memory_size, config.memory_dim)
    ...
    def _update_training_history_cache(
        ...
        if episode_indices is None or frame_indices is None or current_memory_tokens.numel() == 0:
            return

        batch_size = current_memory_tokens.shape[0]
        ...
        cache_limit = max(self.config.memory_size * 8, self.config.memory_size)

        for token, episode_id, frame_id in zip(
            current_memory_tokens.detach().cpu(), episode_ids, frame_ids, strict=True
        ):
            episode_key = int(episode_id)
            frame_key = int(frame_id)
            entries = self.training_history_cache.setdefault(episode_key, [])

            replaced = False
            for idx, (cached_frame, _) in enumerate(entries):
                if cached_frame == frame_key:
                    entries[idx] = (frame_key, token.clone())
                    replaced = True
                    break

            if not replaced:
                entries.append((frame_key, token.clone()))

            entries.sort(key=lambda item: item[0])
            if len(entries) > cache_limit:
                del entries[:-cache_limit]

    def _get_runtime_history(self, batch_size: int, device: torch.device) -> tuple[Tensor | None, Tensor | None]:
        if self.training or not self.config.use_history_memory:
            return None, None
        if len(self.runtime_memory_tokens) == 0:
            return None, None
        if any(token.shape[0] != batch_size for token in self.runtime_memory_tokens):
            return None, None

        history_tokens = torch.stack(self.runtime_memory_tokens, dim=1).to(device=device)
        memory_mask = torch.zeros(
            (batch_size, history_tokens.shape[1]),
            dtype=torch.bool,
            device=device,
        )
    ...
    def _update_runtime_history(self, current_memory_token: Tensor):
        ...
        self.runtime_memory_tokens.append(current_memory_token.detach().clone())
        if len(self.runtime_memory_tokens) > self.config.memory_size:
            self.runtime_memory_tokens.pop(0)
    ...
    def _build_training_history_from_batch(
        ...
        """Build a per-sample history window from cached earlier frames of the same episode."""
    ...
    def _augment_prefix_with_history(
        ...
        current_state = self._summarize_hidden_states(prefix_hidden_states, prefix_pad_masks)
        memory_dtype = self.history_memory_encoder.proj.weight.dtype
        current_state_for_memory = current_state.to(dtype=memory_dtype)
        current_memory_token = self.history_memory_encoder(current_state_for_memory)

        if history_tokens is None:
            if self.training:
                history_tokens, memory_mask = self._build_training_history_from_batch(
                    current_memory_token,
                    episode_indices,
                    frame_indices,
                )
            else:
                history_tokens, memory_mask = self._get_runtime_history(prefix_embs.shape[0], prefix_embs.device)

        history_summary = None
        attn_weights = None
        if history_tokens is not None and history_tokens.shape[1] > 0:
            history_tokens = history_tokens.to(device=current_memory_token.device, dtype=current_memory_token.dtype)
            if memory_mask is not None:
                memory_mask = memory_mask.to(device=current_memory_token.device, dtype=torch.bool)

            if self.memory_time_embedding is not None:
                positions = torch.arange(history_tokens.shape[1], device=history_tokens.device)
                positions = positions.clamp(max=self.config.memory_size - 1)
                history_tokens = history_tokens + self.memory_time_embedding(positions).unsqueeze(0).to(
                    dtype=history_tokens.dtype
                )

            history_summary, attn_weights = self.history_retriever(
                current_state_for_memory.unsqueeze(1),
                history_tokens,
                memory_mask=memory_mask,
            )
            history_summary = history_summary.squeeze(1)
            if self.config.memory_fusion == 'gated':
                fused_state = self.gated_memory_fusion(current_state_for_memory, history_summary)
            else:
                history_hidden = self.gated_memory_fusion.memory_to_hidden(history_summary)
                fused_state = 0.5 * (current_state_for_memory + history_hidden)
            fused_state = fused_state.to(dtype=current_state.dtype)
        else:
            history_summary = torch.zeros(
                current_memory_token.shape[0],
                self.config.memory_dim,
                device=current_memory_token.device,
                dtype=current_memory_token.dtype,
            )
            fused_state = current_state

        prefix_delta = (fused_state - current_state).unsqueeze(1).to(dtype=prefix_embs.dtype)
        prefix_embs = prefix_embs + prefix_delta

        return prefix_embs, current_memory_token, history_summary, attn_weights
    ...
    def sample_actions(
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(images, img_masks, tokens, masks)
        prefix_embs, current_memory_token, _, _ = self._augment_prefix_with_history(
            prefix_embs,
            prefix_pad_masks,
            prefix_att_masks,
        )
        ...
        if current_memory_token is not None:
            self._update_runtime_history(current_memory_token)
        return x_t
